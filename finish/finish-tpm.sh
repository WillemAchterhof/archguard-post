...
ag_finish-tpm()
{
    ...
    [[ -f "$AG_TPM_PUBKEY" ]] \
        || {
            ag_log_tpm "ERROR: PCR signing public key not found: $AG_TPM_PUBKEY"
            return 1
        }
    ...
    ag_log_tpm "Raw PCR policy: $AG_TPM_PCRS"
    ag_log_tpm "Signed PCR policy (UKI integrity): $AG_TPM_PUBKEY"
